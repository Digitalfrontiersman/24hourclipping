    ...
    <section className="relative grain overflow-hidden min-h-[calc(100svh-4rem)] flex items-center" data-testid="hero-banner">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-12 grid lg:grid-cols-2 gap-6 sm:gap-10 lg:gap-8 items-center">
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
          ...
          {/* before / after row — tidy, symmetrical, works at every breakpoint */}
          <div className="mt-4 sm:mt-6 w-full max-w-[560px] flex items-center justify-between gap-3 rounded-xl sm:rounded-2xl bg-white/[0.03] border border-white/10 px-3 sm:px-4 py-2.5 sm:py-3" data-testid="hero-before-after">
            <div className="flex flex-col items-start gap-0.5">
              <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-500">Before</span>
              <span className="text-[10px] sm:text-xs text-zinc-400">45 min VOD</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-12 h-8 sm:w-16 sm:h-10 rounded-md overflow-hidden border border-white/20 grayscale opacity-70"><img src={HERO_POSTER} alt="" className="w-full h-full object-cover object-top" /></div>
              <ArrowRight className="w-4 h-4 text-[#CCFF00]" />
              <div className="w-12 h-8 sm:w-16 sm:h-10 rounded-md overflow-hidden border border-[#CCFF00]/60 shadow-[0_0_14px_rgba(204,255,0,0.25)] relative">
                <img src={HERO_POSTER} alt="" className="w-full h-full object-cover object-top scale-110" />
            ...
            <div className="flex flex-col items-end gap-0.5">
              ...
              <span className="text-[10px] sm:text-xs text-white font-bold">0:45 clip</span>
            </div>
          </div>
        </motion.div>
      </div>
